    ...
    for k = 1 : K
        for lcl = 1 : Ncl
            ...
            for lray = 1 : Nray
                ...
                Hyy1(:, k) = Hyy1(:, k) + alpha_tt * am;
                for n = 1 : Nb
                    ab(n, 1) = 1/sqrt(Nb)*exp(1i*pi*(n-1)*sin(thetabstt));
                end
                H1(:, (k-1)*Nm+1:k*Nm) = H1(:, (k-1)*Nm+1:k*Nm) + alpha_tt * ab * am';
                Hxx1(:, k) = Hxx1(:, k) + alpha_tt * ab;
            end
        end
    end
    %%%%%%%%%%%
    ...
    for k = Ks(1, 1)+1 : K
        for lcl = 1 : Ncl
            ...
            for lray = 1 : Nray
                ...
                alpha_tt = (randn + 1i*randn) / sqrt(2);
                Hyy2(:, k) = Hyy2(:, k) + alpha_tt * am;
                for n = 1 : Nb
                    ab(n, 1) = 1/sqrt(Nb)*exp(1i*pi*(n-1)*sin(thetabstt));
                end
                H2(:, (k-1)*Nm+1:k*Nm) = H2(:, (k-1)*Nm+1:k*Nm) + alpha_tt * ab * am';
                Hxx2(:, k) = Hxx2(:, k) + alpha_tt * ab;
            end
        end
    end
    %%%%%%%%%%%
    ...
    for k = Ks(2, 1)+1 : K
        for lcl = 1 : Ncl
            ...
            for lray = 1 : Nray
                ...
                alpha_tt = (randn + 1i*randn) / sqrt(2);
                Hyy3(:, k) = Hyy3(:, k) + alpha_tt * am;
                for n = 1 : Nb
                    ab(n, 1) = 1/sqrt(Nb)*exp(1i*pi*(n-1)*sin(thetabstt));
                end
                H3(:, (k-1)*Nm+1:k*Nm) = H3(:, (k-1)*Nm+1:k*Nm) + alpha_tt * ab * am';
                Hxx3(:, k) = Hxx3(:, k) + alpha_tt * ab;
            end
        end
    end
    %%%%%%%%%%%
    ...
    for k = Ks(3, 1)+1 : K
        for lcl = 1 : Ncl
            ...
            for lray = 1 : Nray
                ...
                alpha_tt = (randn + 1i*randn) / sqrt(2);
                Hyy4(:, k) = Hyy4(:, k) + alpha_tt * am;
                for n = 1 : Nb
                    ab(n, 1) = 1/sqrt(Nb)*exp(1i*pi*(n-1)*sin(thetabstt));
                end
                H4(:, (k-1)*Nm+1:k*Nm) = H4(:, (k-1)*Nm+1:k*Nm) + alpha_tt * ab * am';
                Hxx4(:, k) = Hxx4(:, k) + alpha_tt * ab;
            end
        end
    end
    %%%%%%%%%%%
    ...
    for k = Ks(4, 1)+1 : K
        for lcl = 1 : Ncl
            ...
            for lray = 1 : Nray
                ...
                alpha_tt = (randn + 1i*randn) / sqrt(2);
                Hyy5(:, k) = Hyy5(:, k) + alpha_tt * am;
                for n = 1 : Nb
                    ab(n, 1) = 1/sqrt(Nb)*exp(1i*pi*(n-1)*sin(thetabstt));
                end
                H5(:, (k-1)*Nm+1:k*Nm) = H5(:, (k-1)*Nm+1:k*Nm) + alpha_tt * ab * am';
                Hxx5(:, k) = Hxx5(:, k) + alpha_tt * ab;
            end
        end
    end
    for nn = 1 : length(Ks)
...
%%%%%%%%%%%%%%%%%%%%%%%%%
plot(Ks, sum_rate(:, 1), 'k--^','LineWidth',1,'MarkerSize',10)
hold on
plot(Ks, sum_rate(:, 2), 'k--s','LineWidth',1,'MarkerSize',10)
plot(Ks, sum_rate(:, 3), 'k--x','LineWidth',1,'MarkerSize',10)
plot(Ks, sum_rate(:, 4), 'k--d','LineWidth',1,'MarkerSize',10)
plot(Ks, sum_rate(:, 5), 'k-*','LineWidth',1,'MarkerSize',10)
plot(Ks, sum_rate(:, 7), 'k-o','LineWidth',1,'MarkerSize',10)
plot(Ks, sum_rate(:, 6), 'k-','LineWidth',1,'MarkerSize',10)
xlim([min(Ks), max(Ks)])
