...
powerle = [40;50;60;70;80];
sigma_s = 10.^(powerle*0.1);%mW
%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
Nb = 256;
Fb = zeros(Nb, Nb);
for n1 = 1 : Nb
...
for ite = 1 : Niten
    ...
    for k = 1 : K
        ...
        H(:, (k-1)*Nm+1:k*Nm) = H(:, (k-1)*Nm+1:k*Nm) / 10^(0.1*pathloss);
        Hxx(:, k) = Hxx(:, k) / 10^(0.1*pathloss);
...
%%%%%%%%%%%%%%%%%%%%%%%%%
plot(powerle, sum_rate(:, 1), 'k--^','LineWidth',1,'MarkerSize',10)
hold on
plot(powerle, sum_rate(:, 2), 'k--s','LineWidth',1,'MarkerSize',10)
plot(powerle, sum_rate(:, 3), 'k--x','LineWidth',1,'MarkerSize',10)
plot(powerle, sum_rate(:, 4), 'k--d','LineWidth',1,'MarkerSize',10)
plot(powerle, sum_rate(:, 5), 'k-*','LineWidth',1,'MarkerSize',10)
plot(powerle, sum_rate(:, 7), 'k-o','LineWidth',1,'MarkerSize',10)
plot(powerle, sum_rate(:, 6), 'k-','LineWidth',1,'MarkerSize',10)
xlim([min(powerle), max(powerle)])
...
set(gca,'XTick',powerle)
xlabel('Transmission power (dBm)','Fontname','Times')
